...
router.get("/users", async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM users WHERE ${whereClause}`);
        const result = await (0, database_1.query)(`SELECT id, email, first_name, last_name, phone, role, created_at,
...
router.put("/users/:id/verify", async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)("UPDATE provider_profiles SET is_verified = true, updated_at = NOW() WHERE user_id = $1 RETURNING *", [id]);
...
router.get("/services", async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM services s WHERE ${whereClause}`);
        const result = await (0, database_1.query)(`SELECT s.*, c.name as category_name, u.first_name, u.last_name
       FROM services s
...
router.get("/bookings", async (req, res) => {
    try {
        ...
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) FROM bookings b WHERE ${whereClause}`);
...
router.post("/categories", [
    ...
    try {
        ...
        const result = await (0, database_1.query)("INSERT INTO categories (name, description, icon) VALUES ($1, $2, $3) RETURNING *", [name, description, icon]);
        res
            .status(201)
            .json({
            success: true,
...
router.get("/analytics", async (req, res) => {
    ...
    `);
        // Top providers
        const topProvidersResult = await (0, database_1.query)(`
      SELECT 
    ...
    `);
        // Service categories
        const categoriesResult = await (0, database_1.query)(`
      SELECT 
        c.name,
