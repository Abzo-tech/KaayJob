...
class ServiceController {
    ...
    static async update(req, res) {
        try {
            ...
            if (updates.categoryId) {
                const category = await prisma_1.prisma.category.findUnique({ where: { id: updates.categoryId } });
                if (!category) {
                    res.status(400).json({ success: false, message: "Catégorie non trouvée" });
                    return;
                }
            ...
            if (updates.priceType !== undefined)
                data.priceType = normalizePriceTypeInput(updates.priceType);
    ...
    static async delete(req, res) {
        try {
            ...
            const activeBookingCount = await prisma_1.prisma.booking.count({
                where: {
                    serviceId: id,
