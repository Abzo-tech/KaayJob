    ...
    static async getMyPayments(req, res) {
        try {
            ...
            const transformedPayments = payments.map((payment) => {
                const activeSubscription = subscriptions.find((sub) => new Date(payment.createdAt) >= new Date(sub.startDate) &&
    ...
    static async updatePaymentStatus(req, res) {
        try {
            ...
            if (!validStatuses.includes(normalizedStatus)) {
                res.status(400).json({
                    success: false,
                    message: "Statut invalide. Valeurs possibles: pending, paid, refunded",
                });
                return;
            }
