...
// ROUTE TEMPORAIRE D'URGENCE: Corriger l'admin en production
// GET /api/auth/fix-admin-production
router.get("/fix-admin-production", async (req: Request, res: Response) => {
  try {
    console.log('🔧 Correction d\'urgence de l\'admin en production...');

    // Supprimer tous les admins existants
    const deleted = await prisma.user.deleteMany({
      where: { role: 'ADMIN' }
    });
    console.log(`Supprimé ${deleted.count} admin(s) existant(s)`);
    ...
    res.json({
      success: true,
      message: 'Admin corrigé en production',
      admin: admin,
      credentials: {
        email: 'admin@kaayjob.com',
        password: 'Password123'
