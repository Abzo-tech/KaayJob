...
router.post("/setup-geolocation", async (req: Request, res: Response) => {
  try {
    ...
    for (const provider of testProviders) {
      ...
      const existingUser = await query('SELECT id FROM users WHERE email = $1', [provider.email]);
...
router.put(
  ...
  [
    ...
    body("bio").optional().isString().withMessage("Bio invalide"),
  ],
  async (req: Request, res: Response) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      ...
      if (planPrice > 0 && paymentMethod) {
        ...
        await createNotification(
          ...
          `Votre paiement de ${planPrice}€ pour l'abonnement ${planSlug.toUpperCase()} a été traité avec succès.`,
          "payment",
