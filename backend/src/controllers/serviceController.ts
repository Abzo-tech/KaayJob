...
export class ServiceController {
  ...
  static async getAll(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Exécuter la requête
      const servicesResult = await query(sqlQuery, params);
      ...
      res.json({
        success: true,
        data: services.map((service: any) => normalizeServiceOutput(service)),
        pagination: {
          page: parsedPage,
          limit: parsedLimit,
          total: total,
  ...
  static async getById(req: Request, res: Response): Promise<void> {
    try {
      ...
      const serviceResult = await query(serviceQuery, [id]);
      ...
      const reviews = reviewsResult.rows.map((reviewRow: any) => ({
        id: reviewRow.id,
  ...
  static async create(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const providerQuery = `
        ...
        WHERE pp.user_id = $1
      `;
      const providerResult = await query(providerQuery, [user.id]);
      ...
      const serviceResult = await query(serviceQuery, [newService.id]);
  ...
  static async update(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (updates.categoryId !== undefined) {
        // Vérifier que la catégorie existe
        const categoryQuery = `SELECT id FROM categories WHERE id = $1`;
        const categoryResult = await query(categoryQuery, [updates.categoryId]);
      ...
      const serviceResult = await query(serviceQuery, [updatedService.id]);
      const row = serviceResult.rows[0];
  ...
  static async delete(req: Request, res: Response): Promise<void> {
    try {
      ...
      `;
      const bookingsResult = await query(bookingsQuery, [id]);
  ...
  static async getByProvider(req: Request, res: Response): Promise<void> {
    try {
      ...
      const servicesResult = await query(servicesQuery, [providerId]);
      const services = servicesResult.rows.map((row: any) => ({
        id: row.id,
        name: row.name,
        description: row.description,
